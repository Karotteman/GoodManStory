...
void AEnemiesManager::Spawn()
{
	if (Spawning)
	{
		...
		Manager.Add(GetWorld()->SpawnActor<ABaseEnemy>(TrashMob->GeneratedClass, Spawners[IndexSpawn]->GetActorLocation(),
		Spawners[IndexSpawn]->GetActorRotation(), SpawnParams));
		NumberMinionToSpawnCurr++;
		IndexSpawn++;
		if(IndexSpawn > Manager.Num()-1)
			IndexSpawn = 0;

		if(NumberMinionToSpawnCurr == NumberMinionToSpawn)
		{
			Spawning = false;
